...
export default function DeleteProductButton({ productId, imageUrls, variant = 'default' }: DeleteProductButtonProps) {
    ...
    return (
      <>
        <div 
          ...
          className="flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-danger/10 text-danger transition-colors w-full rounded-sm"
        >
          <Trash2 className="w-4 h-4" />
          Excluir Produto
        </div>
        {showConfirm && <ModalContent />}
      </>
    );
  }
